...
# Process HELM data
# ──────────────────────────────────────────────
hd_result_path = f"./data/helm/hd/{model_name}"

if not os.path.exists(hd_result_path):
    os.makedirs(hd_result_path, exist_ok=True)

data_path = f"./data/helm/data/{model_name}/data.json"
if not os.path.exists(data_path):
    print(f"HELM data not found at {data_path}")
    print(f"Available model dirs: {os.listdir('./data/helm/data/') if os.path.exists('./data/helm/data/') else 'N/A'}")
    sys.exit(1)
